...
class UserUtility
{
    ...
    public static function storeStock($user, $stockInfo, $shares)
    {
        ...
        if (self::canBuyShares($user, $stockInfo, $shares)) {
            ...
            if ($stocks->count() < $maxStocks) {
                ...
                $portfolio_stock->purchase_price = $stockInfo["data"][0]["close"];
                $portfolio_stock->weighted_price = $stockInfo["data"][0]["close"];
    ...
    {
        $priceUSD = CurrencyConverter::convertToUSD("NGN", $stockInfo["data"][0]["close"]);
    ...
    private static function updateStock($user, $stockInfo, $shares)
    {
        ...
        $sharePrice = $stockInfo["data"][0]["close"];
        $portfolio_stock->share_count += $shares;
